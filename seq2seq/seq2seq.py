...
def generate(text, input_encoding_dict, model, max_input_length, max_output_length, beam_size, max_beams,
    ...
    encoder_input = encode_sequences(
        input_encoding_dict, [text], max_input_length)
    ...
    while len(running_beams) != 0:
        running_beams = sorted(
            running_beams, key=lambda tup: np.prod(tup[1]), reverse=True)
        ...
        for running_beam, probs in running_beams:
            if len(probs) >= min_cut_off_len:
                ...
                for i in range(1, beam_size + 1):
                    ...
                    temp_running_beams.append(
                        [temp_running_beam, probs + [ith_prob]])
...
def infer(text, model, params, beam_size=3, max_beams=3, min_cut_off_len=10, cut_off_ratio=1.5):
    ...
    for decoder_output, probs in decoder_outputs:
        outputs.append({'suggestion': decode_sequence(
            output_decoding_dict, decoder_output[0]), 'grammarModelProb': np.prod(probs)})
...
def infer_greedy(text, model, params):
    ...
    decoder_output = generate_greedy(
        text, input_encoding_dict, model, max_input_length, max_output_length)
...
def build_params(input_data=[], output_data=[], params_path='test_params', max_lengths=(5, 5)):
    ...
    input_encoding, input_decoding, input_dict_size = build_sequence_encode_decode_dicts(
        input_data)
    output_encoding, output_decoding, output_dict_size = build_sequence_encode_decode_dicts(
        output_data)
    params = {}
...
def convert_training_data(input_data, output_data, params):
    ...
    encoded_training_input = encode_sequences(
        input_encoding, input_data, max_input_length)
    encoded_training_output = encode_sequences(
        output_encoding, output_data, max_output_length)
    training_encoder_input = encoded_training_input
    training_decoder_input = np.zeros_like(encoded_training_output)
    training_decoder_input[:, 1:] = encoded_training_output[:, :-1]
    training_decoder_input[:, 0] = char_start_encoding
    training_decoder_output = np.eye(output_dict_size)[
...
def build_model(params_path='test/params', enc_lstm_units=128, unroll=True):
    ...
    output = TimeDistributed(Dense(enc_lstm_units, activation="tanh"))(
